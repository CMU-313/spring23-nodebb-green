...
exports.canEdit =
    exports.isOwnerOrAdminOrMod =
    exports.isAdminOrMod =
    exports.canDelete =
    exports.canPurge =
    exports.filterUids =
    exports.filterTids =
    exports.can =
    exports.get =
    exports.canViewDeletedScheduled =
        void 0;
...
function canViewDeletedScheduled(
    topic,
    privileges = {},
    viewDeleted = false,
    viewScheduled = false
...
async function get(tid, uid) {
    ...
    const deletable =
        ...
        isAdministrator;
    const mayReply = canViewDeletedScheduled(
        topicData,
        {},
        false,
...
async function filterTids(privilege, tids, uid) {
    ...
    const canViewDeleted = lodash_1.default.zipObject(
        cids,
        results.view_deleted
    );
    const canViewScheduled = lodash_1.default.zipObject(
        cids,
        results.view_scheduled
    );
    tids = topicsData
        .filter(
            (t) =>
    ...
    // The next line calls a function in a module that has not been updated to TS yet
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
    const data = await plugins_1.default.hooks.fire(
        "filter:privileges.topics.filter",
        {
            privilege: privilege,
            uid: uid,
            tids: tids,
...
async function filterUids(privilege, tid, uids) {
    ...
    if (topicData.scheduled) {
        // The next line calls a function in a module that has not been updated to TS yet
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
        const canViewScheduled = await helpers_1.default.isUsersAllowedTo(
            "topics:schedule",
            uids,
            topicData.cid
        );
...
async function canDelete(tid, uid) {
    ...
    const [isModerator, isAdministrator, isOwner, allowedTo] =
        await Promise.all([
            ...
            // The next line calls a function in a module that has not been updated to TS yet
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
            helpers_1.default.isAllowedTo("topics:delete", uid, [
                topicData.cid,
            ]),
        ]);
    if (isAdministrator) {
    ...
    return (
        allowedTo[0] &&
        ((isOwner &&
            (parseInt(deleterUid, 10) === 0 || deleterUid === topicData.uid)) ||
            isModerator)
    );
}

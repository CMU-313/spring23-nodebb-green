...
export async function filterTids(
    privilege: string,
    tids: (string | number)[],
    uid: string | number
) {
    ...
    const results = (await privsCategories.getBase(
        privilege,
        cids,
        uid
    )) as Result
...
export async function filterUids(
    privilege: string,
    tid: string | number,
    uids: (number | string)[]
) {
...
export async function canDelete(
    ...
    const [isModerator, isAdministrator, isOwner, allowedTo] =
        await Promise.all([
            ...
            // The next line calls a function in a module that has not been updated to TS yet
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
            helpers.isAllowedTo('topics:delete', uid, [
                topicData.cid,
            ]) as boolean,
        ])
    ...
    // The next line calls a function in a module that has not been updated to TS yet
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
    const { preventTopicDeleteAfterReplies } = meta.config as {
        preventTopicDeleteAfterReplies: number | null
    }
    if (
        !isModerator &&
        ...
        const langKey =
            preventTopicDeleteAfterReplies > 1
                ? // The next line calls a function in a module that has not been updated to TS yet
                  // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
                  `[[error:cant-delete-topic-has-replies, ${
                      meta.config.preventTopicDeleteAfterReplies as string
                  }]]`
...
export async function isOwnerOrAdminOrMod(
    tid: number | string,
    uid: number | string
) {

...
export async function get(caller: Caller, data: Data) {
    ...
    const [userPrivileges, topic]: [privileges, TopicObject] =
        (await Promise.all([
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
...
export async function create(caller: Caller, data: Data) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
    await socketHelpers.emitToUids(
        'event:new_post',
        { posts: [result.postData] },
        [caller.uid]
    )
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
    await socketHelpers.emitToUids('event:new_topic', result.topicData, [
        caller.uid,
    ])
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
...
export async function reply(caller: Caller, data: Data) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
    const postObj: string = (await posts.getPostSummaryByPids(
        [postData.pid],
        caller.uid,
        {}
    )) as string

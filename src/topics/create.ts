...
async function guestHandleValid(data) {
    ...
    if (
        ...
        parseInt(data.uid as string, 10) === 0 &&
        data.handle
    ) {
        // The next line calls a function in a module that has not been updated to TS yet
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
...
export = function (Topics: TopicFields) {
    async function onNewPost(postData: PostObjectPartial, data: TopicData) {
        ...
        const [userInfo, topicInfo]: (UserObjectSlim | TopicObject)[] =
            (await Promise.all([
                // The next line calls a function in a module that has not been updated to TS yet
                // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
                posts.getUserInfoForPosts(
                    [postData.uid],
                    uid
                ) as UserObjectSlim[],
    ...
    Topics.create = async function (data: TopicData) {
        ...
        // The next line calls a function in a module that has not been updated to TS yet
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
        const tid: number = (await db.incrObjectField(
            'global',
            'nextTid'
        )) as number
    ...
    Topics.post = async function (data: TopicData) {
        ...
        const [categoryExists, canCreate, canTag]: boolean[] =
            (await Promise.all([
                // The next line calls a function in a module that has not been updated to TS yet
                // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
        ...
        // The next line calls a function in a module that has not been updated to TS yet
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
        const settings: SettingsObject = (await user.getSettings(
            uid
        )) as SettingsObject
        // The next line calls a function in a module that has not been updated to TS yet
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
        const topics: TopicObject[] = await Topics.getTopicsByTids(
            [postData.tid],
            uid
        )
    ...
    Topics.checkTitle = function (title) {
        ...
        check(
            title,
            meta.config.minimumTitleLength as string,
            meta.config.maximumTitleLength as string,
            'title-too-short',
            'title-too-long'
        )
    }
    ...
    Topics.checkContent = function (content) {
        ...
        check(
            content,
            meta.config.minimumPostLength as string,
            meta.config.maximumPostLength as string,
            'content-too-short',
            'content-too-long'
        )
    }

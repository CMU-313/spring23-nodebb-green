...
export const get = async (req: ExtendedRequest, res: Response) => {
  ...
  await helpers.formatApiResponse(
    200,
    res,
    await api.topics.get(req, req.params)
  );
};
...
async function resolveTopic(tid: string | number, uid: number) {
  ...
  const isOwnerOrAdminOrMod = await privileges.topics.isOwnerOrAdminOrMod(
    tid,
    uid
  );
  ...
  // The next line calls a function in a module that has not been updated to TS yet
  // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
  await plugins.hooks.fire("action:topic.resolve", {
...
export const deleteThumb = async (req: ExtendedRequest, res: Response) => {
  if (!req.body.path.startsWith("http")) {
    middleware.assert.path(req, res, () => {
